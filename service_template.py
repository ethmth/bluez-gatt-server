...
class ReadWriteNotifyCharacteristic(Characteristic):
...
class MqttSrcReadWriteNotifyCharacteristic(ReadWriteNotifyCharacteristic):
    ...
    def __init__(self, bus, index, service, chrc_assigned_number, initial_value, mqtt_topic_url):
        ...
        ReadWriteNotifyCharacteristic.__init__(
            self, bus, index,
            service,
            chrc_assigned_number,            
            initial_value
...
def create_read_write_notify_service(bus, index, service_assigned_number, is_primary, chrc_table_arg):
    ...
    for id, row in chrc_df.iterrows():
        ...
        chrc = MqttSrcReadWriteNotifyCharacteristic(bus, id + 1, service, row.assigned_number, row.default_val_hexdump, row.mqtt_url)
